...
# Constants
DEFAULT_NUM_OFFSPRING = 20
POP_SIZE_PARAMS = [1, 2, 3, 4, 5, 10, 15, 20, 25, 30, 40, 60]
NUM_SCENES = 10
NUM_RESULTS = 10
ACTOR_COUNTS = [4, 2, 3]
OUTPUT_STATS_PATH = "examples/basic/_output/_measurementstats.json"
OUTPUT_EXCEL_PATH = "data/town02-scene-data.xlsx"
...
def collect_scene_data(
    ...
    json_output_path = f"data/town02-{num_actors}-actor-scene-{scene_id}-pop-{pop_size}.json"
...
def main() -> None:
    for scene_idx in range(NUM_SCENES):
        for pop_size in POP_SIZE_PARAMS:
            batch_data = []

            for num_actors in ACTOR_COUNTS:
                rows = collect_scene_data(scene_idx, num_actors, pop_size, DEFAULT_NUM_OFFSPRING)
